...
class MeanQuadraticFunction(BaseMeanFunction):
    def __init__(self, As, bs, seed):
        super(MeanQuadraticFunction, self).__init__(seed, len(As))
        self._quadratic_functions = [QuadraticFunction(A, b) for A, b in zip(As, bs)]
    ...
    def batch_gradient_at_points(self, points, batch_size):
        ...
        for point in points:
            ...
            for index in indices:
                output_gradient = output_gradient + self._quadratic_functions[index].gradient(point)
...
class TridiagonalQuadraticFunction(BaseFunction):
    ...
    def smoothness_variance_bound_functions(functions):
        ...
        square_matrices = list(map(lambda_square_matrix, matrices))
        mean_square_matrices = lambda_mean_matrices(square_matrices)
        svdvals = scipy.sparse.linalg.svds(mean_square_matrices - square_mean_matrices,
                                           k=1, return_singular_vectors=False)
        op_norm = np.max(svdvals)
        svb = np.sqrt(op_norm)
        return svb
...
class NonConvexLossFunction(BaseGradientMLFunction):
    def __init__(self, features, labels, seed=None, check_accuracy=False):
...
class LogisticRegressionFunction(BaseGradientMLFunction):
    ...
    def liptschitz_gradient_constant(self):
        ...
        square_norm = np.sum(np.square(self._features_np), axis=1)
        assert len(square_norm) == self._features_np.shape[0]
        liptschitz_constant = bound_for_second_derivative * np.mean(square_norm)
        return liptschitz_constant
